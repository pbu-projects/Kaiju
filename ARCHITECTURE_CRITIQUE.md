...
The documentation across [README#L1-L42](README.md#L1-L42), [roadmap#L1-L25](roadmap.md#L1-L25), [domain#L1-L34](docs/domain.md#L1-L34), and [DB#L1-L115](database/DB.md#L1-L115) defines an ambitious, GIS-first volunteer management platform. 
...
[architecture#L14-L18](docs/architecture.md#L14-L18) prescribes two distinct frontend technologies:
...
Early architecture notes discussed GraalVM Native Image compilation ([build.gradle.kts#L65-L96](build.gradle.kts#L65-L96), [architecture#L19-L26](docs/architecture.md#L19-L26)).
...
In Phase 1, endpoints are currently scaffolded open without security filters to facilitate database seeding ([fetch-global-test-locations](database/init/fetch-global-test-locations.py)) and rapid test iteration.
...
* As Authentik JWT authentication and role-based access control are layered on:
  ...
  * Role definitions across database constraints ([01-schema#L7-L14](database/init/01-schema.sql#L7-L14)) and Java enums ([UserRole#L5-L12](src/main/java/lol/pbu/kaiju/model/UserRole.java#L5-L12)) must remain unified.
...
3. **Secret Hygiene**:
   * Ensure local environment secrets in [.env#L1](.env#L1) are properly excluded from version control via [.gitignore#L1-L38](.gitignore#L1-L38).
