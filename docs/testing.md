...
We use [Spock](https://spockframework.org/) and [Testcontainers](https://testcontainers.com/) to validate behavior against real services.
...
- **Explicit Boundary Checks**: When testing radius searches (`ST_DWithin`) or region containment (`ST_Intersects`), always test both matching coordinates and out-of-bounds coordinates to prove exclusion.
- **Deterministic Ordering**: When asserting on paginated queries, include a secondary deterministic sort (such as `ORDER BY id ASC`) to avoid tie-breaking flakiness.
- **International Address Fixtures**: Test realistic global formats, including alphanumeric postal codes (UK/Canada) and locations without postal codes.
