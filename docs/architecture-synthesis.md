...
- **The Precedence Evaluated**:
  While the critique labeled this an anti-pattern, Counterpoints correctly defends it as an **ingestion optimization**. Skipping pre-flight `existsById` queries doubles throughput during bulk synchronization and seed script execution ([08-global-locations](../database/init/08-global-locations.sql)). This is a sound, pragmatic precedent for administrative ingestion routes.
